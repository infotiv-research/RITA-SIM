...
from curobo_msgs.srv import AddObject, RemoveObject, GetVoxelGrid, SetCollisionCache, UpdateObjectPoses
...
class ObstacleManager:
    ...
    def __init__(self, node, config_manager, initial_world_cfg: 'WorldConfig' = None):
        ...
        # Store derived cuboid centers in mesh-local coordinates so moved mesh
        # obstacles can update their cuboid approximation without re-voxelizing.
        self.mesh_cuboid_local_offsets = {}
    ...
    def add_object(self, node, request: AddObject, response):
        ...
        match request.type:
            ...
            case request.MESH:
                ...
                try:
                    mesh_rotation = Rotation.from_quat(
                        [
                            extracted_pose[4],
                            extracted_pose[5],
                            extracted_pose[6],
                    ...
                    mesh_translation = np.array(extracted_pose[:3], dtype=float)
                    ...
                    for idx, (origin, size) in enumerate(cubes):
                        ...
                        local_center = mesh_rotation.inv().apply(center - mesh_translation)
                        cuboid_local_offsets.append(
                            {
                                "name": cuboid_name,
                                "offset": local_center.tolist(),
                            }
                        )
                    ...
                    self.mesh_cuboid_local_offsets[request.name] = cuboid_local_offsets
    ...
    def remove_object(self, node, request: RemoveObject, response):
        ...
        if request.name in self.mesh_cuboid_mapping:
            ...
            self.mesh_cuboid_local_offsets.pop(request.name, None)
    ...
    @staticmethod
    def _pose_msg_to_list(pose_msg):
        return [
    ...
    def _set_object_pose(self, obj, pose_list):
        obj.pose = [float(value) for value in pose_list]

    def _update_mesh_derived_cuboids(self, mesh_name, pose_list):
        offsets = self.mesh_cuboid_local_offsets.get(mesh_name)
        if not offsets:
            return

        cuboids_by_name = {obj.name: obj for obj in self.world_cfg.cuboid}
        mesh_rotation = Rotation.from_quat(
            [pose_list[4], pose_list[5], pose_list[6], pose_list[3]]
        )
        mesh_translation = np.array(pose_list[:3], dtype=float)
        cuboid_orientation = [pose_list[3], pose_list[4], pose_list[5], pose_list[6]]

        for offset_entry in offsets:
            cuboid = cuboids_by_name.get(offset_entry["name"])
            if cuboid is None:
                continue
            center = mesh_translation + mesh_rotation.apply(
                np.array(offset_entry["offset"], dtype=float)
            )
            cuboid.pose = [
                float(center[0]),
                float(center[1]),
                float(center[2]),
                float(cuboid_orientation[0]),
                float(cuboid_orientation[1]),
                float(cuboid_orientation[2]),
                float(cuboid_orientation[3]),
            ]
    ...
    def update_object_poses(
        ...
        response: UpdateObjectPoses.Response,
    ):
        """Update poses of existing obstacles without removing and re-adding them."""
        names = list(request.names)
        poses = list(request.poses)

        if len(names) != len(poses):
            response.success = False
            response.message = "names and poses must have the same length"
            response.updated_count = 0
            response.failed_names = list(names)
            return response

        if not names:
            response.success = True
            response.message = "No obstacle poses to update"
            response.updated_count = 0
            response.failed_names = []
            return response
        ...
        for object_name, pose_msg in zip(names, poses):
            if object_name in seen_names:
                failed_names.append(str(object_name))
                continue
            seen_names.add(object_name)

            try:
                object_info = self.get_object(str(object_name))
            except ValueError:
                failed_names.append(str(object_name))
                continue

            pose_list = self._pose_msg_to_list(pose_msg)
            self._set_object_pose(object_info["object"], pose_list)
            if object_info["type"] == "mesh":
                self._update_mesh_derived_cuboids(str(object_name), pose_list)
            updated_count += 1

        response.updated_count = int(updated_count)
        response.failed_names = failed_names
        response.success = not failed_names
        if failed_names:
            response.message = (
                f"Updated {updated_count} obstacles; failed to update: "
                + ", ".join(failed_names)
            )
        else:
            response.message = f"Updated {updated_count} obstacle poses"
        return response
