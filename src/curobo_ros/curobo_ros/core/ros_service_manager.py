...
from curobo_msgs.srv import AddObject, RemoveObject, GetVoxelGrid, GetCollisionDistance, SetCollisionCache, GetRobotStrategies, SetLinkCollision, AttachObject, UpdateObjectPoses
...
class RosServiceManager:
    ...
    def __init__(self, node, obstacle_manager, robot_model_manager, config_manager, config_wrapper=None, robot_context=None):
        ...
        self.update_object_poses_srv = None
    ...
    def init_services(self):
        ...
        self.update_object_poses_srv = self.node.create_service(
            UpdateObjectPoses,
            self.node.get_name() + '/update_object_poses',
            partial(self._callback_update_object_poses, self.node)
        )
        ...
        # Service to attach/detach kinematic collision spheres to robot links
        self.attach_object_srv = self.node.create_service(
            AttachObject,
            self.node.get_name() + '/attach_object',
            self.robot_model_manager.attach_object_callback
        )
    ...
    def _callback_update_object_poses(self, node, request: UpdateObjectPoses.Request, response: UpdateObjectPoses.Response):
        """Delegate batched pose-only obstacle updates and refresh world once."""
        response = self.obstacle_manager.update_object_poses(node, request, response)

        if response.updated_count > 0 and self.config_wrapper is not None:
            self.config_wrapper.update_world_config(node, log_summary=False)
